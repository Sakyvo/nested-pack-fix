...
final class WarningEntryRenderer {
    ...
    static void draw(
        ...
        int packFormat = entry.nestedpackfix$getPackFormat();
        if (packFormat != 1) {
            Gui.drawRect(x - 1, y - 1, x + width - 9, y + height + 1, -8978432);
        }
        ...
        boolean controlsVisible =
            (minecraft.gameSettings.touchscreen || hovered)
                && entry.nestedpackfix$isUsable();
        drawMovementControls(entry, x, y, mouseX, mouseY, controlsVisible);
        ...
        String title = entry.nestedpackfix$getTitle();
        String description = entry.nestedpackfix$getDescription();
        if (controlsVisible && packFormat != 1) {
            title = I18n.format("resourcePack.incompatible");
        ...
        title = trimTitle(minecraft, title, textWidth);
        minecraft.fontRendererObj.drawStringWithShadow(
            title,
            textX,
            y + 1,
